...
int menu_item_trigger_event(menu_item_t *item, menu_event_t event, void **event_data){
    ...
    switch(event){
        ...
        if(menu_ctx.tooltip){
            *menu_ctx.tooltip = item->tooltip;
        }
        return true;
        case MENU_EVENT_EXIT:
        if(menu_ctx.tooltip){
            *menu_ctx.tooltip = NULL;
        }
        return true;
        default:
        return false;
...
int menu_return(event_handler_t *handler, menu_event_t event, void **event_data){
    menu_trigger_event(menu_ctx.menu, MENU_EVENT_RETURN, event_data);
    return 1;
}
